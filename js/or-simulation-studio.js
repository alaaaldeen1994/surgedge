/**
 * SurgEdge 360° Interactive Operating Room (OR) WebGL Simulation Studio
 * Full 360° Panoramic Theatre Orbit, Dynamic Zoom In/Out, Holographic AR Trajectory, and Intra-Operative Telemetry
 */
...
class ORSimulationStudio {
    constructor() {
        ...
        // 3D Objects & Panoramas
        this.orPanoramaSphere = null;
        this.arHoloGroup = null;
        this.laserScanPlane = null;
        this.holoRingCyan = null;
        this.holoRingAmber = null;

        // Zoom & Orbit State
        this.defaultFov = 52;
        this.currentFov = 52;
        this.minFov = 18;  // Close-up deep zoom into surgical incision
        this.maxFov = 80;  // Wide panoramic OR theatre view
        ...
        this.steps = [
            { id: 'step-1', time: 0, name: '1. Anatomic Landmark Mapping', angle: '8.4° Varus Deformity', cut: 'Pre-Resection', balance: 'Medial Overload (Varus)' },
            { id: 'step-2', time: 120, name: '2. Intramedullary Rod Insertion', angle: '5.8° Valgus Offset', cut: 'Portal Calibrated', balance: 'Axis Aligned (45%)' },
            { id: 'step-3', time: 260, name: '3. Resection Plane Calibration', angle: '0.4° Coronal Deviation', cut: '9.0mm Distal Femur Cut', balance: 'Pins Anchored (85%)' },
            { id: 'step-4', time: 380, name: '4. Joint Realignment & Balance', angle: '0.2° Varus / 3.0° Flex', cut: '9.0mm Distal / 8.5mm Tibial', balance: '99.8% Symmetrical Balance' }
        ];
    ...
    init() {
        ...
        this.camera.position.set(0, 0, 0.1); // Positioned inside sphere looking out
        ...
        if (typeof THREE.OrbitControls !== 'undefined') {
            ...
            this.controls.rotateSpeed = -0.55; // Natural drag-to-look direction
            this.controls.enableZoom = false; // We handle smooth FOV zoom directly for panoramic spheres
        ...
        // 6. Build 360° Operating Room Spherical Environment
        this.build360PanoramaEnvironment();

        // 7. Build Glowing Holographic AR Trajectory Guides (Authentic Surgical Anatomy Only)
        this.build3DHolographicARGuides();

        // 8. Setup Controls, Zoom & Telestration
        this.setupControls();
    ...
    build3DHolographicARGuides() {
        // Holographic AR Overlays positioned in 3D space above the authentic operative knee incision
        this.arHoloGroup = new THREE.Group();
        this.arHoloGroup.position.set(0.1, -1.2, -7.2);

        // 1. Cyan Rotating Coronal Guidance Disc
        const discCyanGeo = new THREE.RingGeometry(0.2, 1.6, 36);
        const discCyanMat = new THREE.MeshBasicMaterial({
            color: 0x38BDF8,
            transparent: true,
            opacity: 0.75,
            side: THREE.DoubleSide
        });
        this.holoRingCyan = new THREE.Mesh(discCyanGeo, discCyanMat);
        this.holoRingCyan.position.set(0, 0.4, 0);
        this.holoRingCyan.rotation.x = Math.PI / 2.3;
        this.arHoloGroup.add(this.holoRingCyan);

        // 2. Amber Rotating Valgus Guidance Ring
        const ringAmberGeo = new THREE.TorusGeometry(1.4, 0.035, 16, 48);
        const ringAmberMat = new THREE.MeshBasicMaterial({
            color: 0xFBBF24,
            transparent: true,
            opacity: 0.9,
            side: THREE.DoubleSide
        });
        this.holoRingAmber = new THREE.Mesh(ringAmberGeo, ringAmberMat);
        this.holoRingAmber.position.set(0, 0, 0.1);
        this.holoRingAmber.rotation.x = Math.PI / 2;
        this.arHoloGroup.add(this.holoRingAmber);

        // 3. Scanning Laser Resection Plane
        const laserPlaneGeo = new THREE.PlaneGeometry(2.5, 1.6);
        ...
        this.laserScanPlane = new THREE.Mesh(laserPlaneGeo, laserPlaneMat);
        this.laserScanPlane.position.set(0, 0.2, 0.1);
        this.laserScanPlane.rotation.x = Math.PI / 2;
        this.arHoloGroup.add(this.laserScanPlane);

        this.scene.add(this.arHoloGroup);
    }
        ...
        document.querySelectorAll('.or-cam-btn[data-cam]').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                if (camMode === 'or-wide') {
                    ...
                    this.setZoomFov(32); // Zoom in close to the knee incision
                    this.controls.target.set(0.05, -0.2, -1);
                }
                this.controls.update();
            });
        });
    ...
    animate() {
        ...
        // 1. Handle OrbitControls & Continuous 360° Auto-Orbit
        if (this.controls) {
            this.controls.update();

            if (this.isAutoOrbiting && this.isPlaying) {
                const angle = 0.0025 * this.playbackSpeed;
        ...
        // 2. Animate 3D Holographic Guides
        if (this.isPlaying) {
            ...
            if (this.holoRingCyan) this.holoRingCyan.rotation.z += 0.04 * this.playbackSpeed;
            if (this.holoRingAmber) this.holoRingAmber.rotation.z -= 0.03 * this.playbackSpeed;

            if (this.laserScanPlane) {
                this.laserScanPlane.position.y = 0.2 + Math.sin(this.currentTime * 3) * 0.25;
            }
        ...
        // 4. Render 2D ECG & Telestration Canvas
    ...
    render2DOverlays() {
        ...
        // Draw Live Dynamic ECG Oscilloscope
        const ecgBoxX = 16;
        const ecgBoxY = h - 60;
